...
class Mock {
    ...
    public fun getPhrase(id: Int): String {
        if (id != 1) {
            val filtered = phrasesList.filter { it.categoryId == id }
            return filtered[Random.nextInt(until = filtered.size)].description
        }
        return phrasesList[Random.nextInt(until = phrasesList.size)].description
    }
}